...
- **Universal Admin Password Log Deletion Fix (`AdminAuthModal.tsx` / `WaterTracker.tsx` / `VitalsTracker.tsx` / `MedicationTracker.tsx`)**: Fixed root cause where Admin password validation previously failed if `adminProfile.pin` was encrypted or out of sync with `localStorage`. Enhanced `isAdminValid` in `AdminAuthModal.tsx` to evaluate cleartext, default `1234`, `localStorage`, and PII-decrypted `adminProfile.pin` payloads. Entering the Admin Password now reliably authorizes deletion across **Water Logs**, **Glucose Readings**, **Blood Pressure Readings**, and **Medication Doses / Prescriptions**. Added BP log deletion capability (`deleteBPLog` and `deleteBPLogDB`).
...
- **PII Credential Encryption & PIN Masking (`piiSecurity.ts` / `supabase.ts` / `AdminView.tsx`)**: Classified user PINs as PII Sensitive Credentials. Built a lightweight AES/XOR cipher utility (`src/utils/piiSecurity.ts`) encrypting PIN payloads into `PII_ENC:...` ciphertext before storing in Supabase or LocalStorage. Masked all PIN displays as `••••` with a `🔒 PII Sensitive` badge tag by default, requiring Master Admin Passcode authentication to decrypt and reveal cleartext PINs in Admin Hub. Added PostgreSQL `COMMENT ON COLUMN public.profiles.pin` security tag.
- **In-App User Guide & Documentation Hub (`UserGuideModal.tsx` / `Header.tsx`)**: Created a dedicated, simple, categorized User Documentation & Help Guide modal (`UserGuideModal.tsx`) covering Prescriptions, Diabetes & Blood Sugar, Blood Pressure, Calendar Timelines, Hydration, Reports Sharing, and PIN Privacy. Added a top header bar button (`📖 User Guide`) for instant access from anywhere in the application.
- **Strict Single-PIN Verification & Reset to Default (`piiSecurity.ts` / `AdminAuthModal.tsx` / `ChangePinModal.tsx` / `AdminView.tsx`)**: Resolved RCA bug where default `"1234"` acted as an unintended global fallback. Enforced strict single-PIN authentication so changing a PIN strictly replaces `"1234"`. Added a **`🔄 Reset PIN to Default (1234)`** action button in `ChangePinModal.tsx` and `AdminView.tsx` enabling users and admins to revert any profile PIN back to `"1234"` with 1-click.

## [1.17.0] - 2026-08-06

### Added & Enhanced
- **Admin/Aditya Exclusive Developer & Database Visibility (`Header.tsx` / `Navigation.tsx` / `AdminView.tsx`)**: Restricted Developer Settings, Supabase Live connection status, and Admin Control Hub to the **Aditya** (or Admin) profile only. Removed Admin tab from the main health navigation bar and relocated the **Admin Hub (`🛡️ Admin Hub`)** to the top header profile bar. Non-admin users see a clean user dashboard with zero exposure to administrative or developer tools.
